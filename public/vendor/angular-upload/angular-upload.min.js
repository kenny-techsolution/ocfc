...
angular.module("lr.upload", ["lr.upload.formdata", "lr.upload.iframe", "lr.upload.directives"]), angular.module("lr.upload.directives", []), angular.module("lr.upload.directives").directive("uploadButton", ["upload", function (a) {
	return{restrict: "EA", scope: {data: "=?data", url: "@", param: "@", method: "@", onUpload: "&", onSuccess: "&", onError: "&", onComplete: "&"}, link: function (b, c, d) {
		var e = angular.element(c), f = angular.element('<input type="file" />');
		if (e.append(f), f.on("change", function () {
			if (!f[0].files || 0 !== f[0].files.length) {
				var c = {url: b.url, method: b.method || "POST", forceIFrameUpload: b.$eval(d.forceIframeUpload) || !1, data: b.data || {}};
				c.data[b.param || "file"] = f, b.$apply(function () {
					b.onUpload({files: f[0].files})
				}), a(c).then(function (a) {
			...
			var d = b.$eval(a);
			f.attr("required", angular.isUndefined(d) || d), c.toggleClass("ng-valid", !d), c.toggleClass("ng-invalid ng-invalid-required", d)
		}), d.$observe("accept", function (a) {
			f.attr("accept", a)
		}), a.support.formData) {
			var g = function () {
	...
	return function (c) {
		return c.transformRequest = b, c.headers = angular.extend(c.headers || {}, {"Content-Type": void 0}), a(c)
	}
	...
	var e = {fileInput: !(new RegExp("(Android (1\\.[0156]|2\\.[01]))|(Windows Phone (OS 7|8\\.0))|(XBLWP)|(ZuneWP)|(WPDesktop)|(w(eb)?OSBrowser)|(webOS)|(Kindle/(1\\.0|2\\.[05]|3\\.0))").test(a.navigator.userAgent) || angular.element('<input type="file">').prop("disabled")), fileUpload: !(!a.XMLHttpRequestUpload || !a.FileReader), formData: !!a.FormData};

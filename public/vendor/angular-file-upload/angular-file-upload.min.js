...
!function (a, b) {
	...
	return b.directive("ngFileDrop", ["$fileUploader", function (b) {
		"use strict";
		return{link: b.isHTML5 ? function (a, b, c) {
			b.bind("drop", function (b) {
				...
				c.target === b[0] && a.$broadcast("file:removeoverclass")
			})
		} : a.noop}
	...
	}), b.directive("ngFileSelect", ["$fileUploader", function (a) {
		"use strict";
		return{link: function (b, c, d) {
			a.isHTML5 || c.removeAttr("multiple"), c.bind("change", function () {
				var e = a.isHTML5 ? this.files : this, f = b.$eval(d.ngFileSelect);
				b.$emit("file:add", e, f), a.isHTML5 && c.attr("multiple") && c.prop("value", null)
			}), c.prop("value", null)
		}}
	}]), b.factory("$fileUploader", ["$compile", "$rootScope", "$http", "$window", function (b, c, d, e) {
		"use strict";
		function f(b) {
			a.extend(this, {scope: c, url: "/", alias: "file", queue: [], headers: {}, progress: null, autoUpload: !1, removeAfterUpload: !1, method: "POST", filters: [], formData: [], isUploading: !1, queueLimit: Number.MAX_VALUE, withCredentials: !1, _nextIndex: 0, _timestamp: Date.now()}, b), this.filters.unshift(this._queueLimitFilter), this.filters.unshift(this._emptyFileFilter), this.scope.$on("file:add", function (a, b, c) {
		...
		function g(c) {
			if (!f.prototype.isHTML5) {
				var d = a.element(c.file), e = b(d.clone())(c.uploader.scope), g = d.val();
				c.file = {lastModifiedDate: null, size: null, type: "like/" + g.slice(g.lastIndexOf(".") + 1).toLowerCase(), name: g.slice(g.lastIndexOf("/") + g.lastIndexOf("\\") + 2)}, c._input = d, e.prop("value", null), d.css("display", "none").after(e)
			}
			a.extend(this, {isReady: !1, isUploading: !1, isUploaded: !1, isSuccess: !1, isCancel: !1, isError: !1, progress: null, index: null}, c)
		}

		return f.prototype = {constructor: f, _emptyFileFilter: function (b) {
		...
		}, isHTML5: !(!e.File || !e.FormData), addToQueue: function (b, c) {
			var d = this.queue.length, e = "length"in b ? b : [b];
			a.forEach(e, function (b) {
				var d = this.filters.length ? this.filters.every(function (a) {
					return a.call(this, b)
				}, this) : !0, e = new g(a.extend({url: this.url, alias: this.alias, headers: a.copy(this.headers), formData: a.copy(this.formData), removeAfterUpload: this.removeAfterUpload, withCredentials: this.withCredentials, method: this.method, uploader: this, file: b}, c));
			...
			c.cancel && c.cancel(), c._destroy && c._destroy(), this.queue.splice(b, 1), this.progress = this._getTotalProgress()
		}, clearQueue: function () {
			for (; this.queue.length;)this.queue[this.queue.length - 1].remove()
			...
			this.trigger("beforeupload", b), b.formData.forEach(function (b) {
				...
				var a = e._transformResponse(c.response), d = e._isSuccessCode(c.status) ? "success" : "error";
			...
			b._form && b._form.replaceWith(e), b._form = c, this.trigger("beforeupload", b), e.prop("name", b.alias), b.formData.forEach(function (b) {
				...
				var a = d[0].contentDocument.body.innerHTML, c = {response: a, status: 200, dummy: !0}, e = f._transformResponse(c.response);
				f.trigger("in:success", c, b, e), f.trigger("in:complete", c, b, e)
			}), c.abort = function () {
				var a = {status: 0, dummy: !0};
		...
		}}, g.prototype = {constructor: g, remove: function () {
